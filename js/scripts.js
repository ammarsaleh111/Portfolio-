...
const CONFIG = {
  formSubmit: {
    endpoint: 'https://formsubmit.co/ajax/ammarsaleh890@gmail.com'
  },
  cvDownload: {
...
const contactForm = {
  ...
  async submit() {
    ...
    const endpoint = CONFIG.formSubmit?.endpoint;

    if (!endpoint) {
      console.error('FormSubmit endpoint is missing in CONFIG.');
      this.showStatus('error', 'Message service is not configured. Please email me directly.');
      return;
    }
    ...
    ['_subject', '_captcha', '_template', '_autoresponse'].forEach((fieldName) => {
    ...
    try {
      ...
      console.error('Form submission error (FormSubmit):', error);
      this.showStatus(
        'error',
        'Unable to send your message right now. Please email me directly at ammarsaleh890@gmail.com.'
      );
    } finally {
